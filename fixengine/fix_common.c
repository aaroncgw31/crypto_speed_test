...
#include "auth/gdax_auth.h"
#include "compat.h"

#include <string.h>
#include <stdio.h>

//extern void LogonMsgGen(const char *time, const char *sender_id, const char *target_id, const char *password, const char *api_secret, char *msg);
...
int fix_session_logon(struct fix_session *session)
{
	...
	char msg[64];
	LogonMsgGen(session->str_now, session->sender_comp_id, session->target_comp_id, session->password, session->api_secret, msg);
